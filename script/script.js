...
for (let user of users) {
  ...
  if (user.unreadMessages > 0) {
    lastMessage.innerHTML =
      '<span class="unreadMessage">last message</span><span class="badge unreadMessagesCount badge-pill">' +
      user.unreadMessages +
      "</span>";
  } else {
...
let getAllStatus = () => {
  ...
  for (let user of users) {
    if (user.status.present) {
      let listNode = document.createElement("a");
      listNode.className =
        "list-group-item list-group-item-action flex-column align-items-start px-3";
      ...
      let col2Div = document.createElement("div");
      col2Div.className = "col-2 align-self-center text-center";
      rowDiv.appendChild(col2Div);
